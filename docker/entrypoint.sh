...
# Export PORT so nginx config substitution can use it
export PORT="${PORT:-80}"
...
echo "Starting Al-Nader Real Estate Application on port $PORT..."

# -----------------------------------------------------------------------------
# Substitute PORT placeholder in nginx config (Railway assigns a dynamic port)
# -----------------------------------------------------------------------------
if [ -f /etc/nginx/http.d/default.conf.template ]; then
    sed "s/PORT_PLACEHOLDER/${PORT}/g" /etc/nginx/http.d/default.conf.template > /etc/nginx/http.d/default.conf
fi
